...
# from latitude, longitude to cartesian coordinate
Latlong_to_Cartesian = function(latlong)
{
   n = nrow(latlong)
   y = matrix(0, n, 3)
   Lat = (90 - latlong[, 1]) * pi / 180
   Long = latlong[, 2] * pi / 180
   for (i in 1 : n)
      y[i, ] = spherical_to_cartesian(c(Lat[i], Long[i]))
   return(y)
}

# from cartesian coordinate to latitude, longitude
cartesian_to_latlong = function(x)
{
   theta = -Acos(x[3])
   phi = Acos(x[1] / sin(theta))
   # lat = 180 / pi * theta
   # long = 90 - 180 / pi * phi
   long = phi * 180 / pi
   lat = theta * 180 / pi + 90
   spherical_coordinate = c(lat, long)
   return(spherical_coordinate)
}
...
{
   n = nrow(x)
   latlong = matrix(0, n, 2)
   for (i in 1 : n)
      latlong[i, ] = cartesian_to_latlong1(x[i, ])
   return(latlong)
}

# from cartesian coordinate to latitude, longitude
cartesian_to_latlong1 = function(x)
{
   theta = Acos(x[3])
   phi = Atan(x[2], x[1])
   # lat = 180 / pi * theta
...
Cartesian_to_Latlong_ele = function(x)
{
   n = nrow(x)
   latlong = matrix(0, n, 2)
   for (i in 1 : n)
      latlong[i, ] = cartesian_to_latlong(x[i, ])
   return(latlong)
}

sphere_plot = function()
{
   number_grid = 100
